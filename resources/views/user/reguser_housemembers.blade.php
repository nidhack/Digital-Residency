...
<?php
$id = Auth::user()->id;
$st=DB::select("select u_reg_hno from tbl_reg_user where  tbl_reg_user.id='$id'");
foreach($st as $row)
   {
$x=$row->u_reg_hno;
 $sb=DB::select("select * from tbl_housemembers_datails where  tbl_housemembers_datails.home_mem_houseno='$x'");
   foreach($sb as $row)
    {
  ?>
 		<div class="message warning"  style="float:left;width:500px;">
			<div class="inset agile">
				<div class="sap_tabs w3ls-tabs" style="width:300px;height:700px;">
					...
					<form method="post" action="{{url('updates/'.$row->home_mem_houseno)}}" enctype="multipart/form-data">
	  @csrf
...
<label><font color=" #ff0080">First name</font></label>
				            <input  type="text" class="form-control"  name="first_name" id="first_name" placeholder="first name"  value="<?php echo $row->home_mem_name; ?>"/><br>
																 
<label><font color=" #ff0080">Occupation</font></label>
                            <input  type="text" class="form-control"name="last_name" id="last_name" placeholder="last name" value="<?php echo $row->home_mem_occupation; ?>"/><br>
              
	<label><font color=" #ff0080">Date Of Birth</font></label>
                            <input  type="text" class="form-control"name="last_name" id="last_name" placeholder="last name" value="<?php echo $row->home_mem_d_o_b; ?>"/></><br>
