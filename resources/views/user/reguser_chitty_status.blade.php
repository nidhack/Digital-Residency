...
<?php
  ...
  $id=Auth::user()->id;
$st=DB::select("SELECT * FROM tbl_chitty_join where id='$id' ORDER BY id DESC Limit 3");
foreach($st as $row)
{
$a=$row->chitty_id ;
$sv=DB::select("SELECT * FROM tbl_chitty_details where 	chitty_id=$a");
foreach($sv as $row)
{
 		...
 		<div class="message warning"  style="float:left;width:500px;">
			<div class="inset agile">
				<div class="sap_tabs w3ls-tabs" style="width:300px;">
                        ...
                        <form action="/chittystatusview?id={{$row->chitty_id}}" method="post">
                        <input type="hidden" name="_token" value="{{@csrf_token()}}"/>
                        @csrf
                        <input  type="hidden" class="form-control" disabled id="chitty_id" name="chitty_id" value="<?php echo $row->chitty_id ?>"/><br>
												<input  type="text" class="form-control" disabled id="chitty_name" name="chitty_id" value="<?php echo $row->chitty_name ?>"/><br>
                        ...
                        <div id="horizontalTab" style="display: block; width: 100%; margin: 0px;">
                            <ul class="resp-tabs-list">
						        ...
						        <input type="submit" class="btn btn-success" name="submit" value="chitty status">
</form>   </div>
                </div>
            </div>
        </div>
